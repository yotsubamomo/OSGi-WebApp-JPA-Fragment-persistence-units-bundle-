 ...
 */
public class MyPersistenceUnitManager
	...
	public void setPersistenceXmlLocations(String[] persistenceXmlLocations) {
		List a = Arrays.asList(persistenceXmlLocations);
		logger.info("setPersistenceXmlLocations -> " + persistenceXmlLocations);
		logger.info("setPersistenceXmlLocations -> " + a);
	...
	public void preparePersistenceUnitInfos() {
		logger.info("<>preparePersistenceUnitInfos start......");
		this.persistenceUnitInfoNames.clear();
		this.persistenceUnitInfos.clear();
		MySpringPersistenceUnitInfo[] puis = readPersistenceUnitInfos();
		logger.info("<> puis = " + puis);
		
		for (MySpringPersistenceUnitInfo pui : puis) {
			logger.info("<> pui = "+ pui);
	 ...
	 */
	private MySpringPersistenceUnitInfo[] readPersistenceUnitInfos() {
		logger.info("<>readPersistenceUnitInfos .....");
		MyPersistenceUnitReader reader = new MyPersistenceUnitReader(this.resourcePatternResolver, this.dataSourceLookup);
		logger.info("<>reader.readPersistenceUnitInfos(this.persistenceXmlLocations) ....."+reader.readPersistenceUnitInfos(this.persistenceXmlLocations));
	...
	protected final MutablePersistenceUnitInfo getPersistenceUnitInfo(String persistenceUnitName) {
		...
		if (pui != null && Proxy.isProxyClass(pui.getClass())) {
			// JPA 2.0 PersistenceUnitInfo decorator with a MySpringPersistenceUnitInfo as target
			Jpa2PersistenceUnitInfoDecorator dec = (Jpa2PersistenceUnitInfoDecorator) Proxy.getInvocationHandler(pui);
			return dec.getTarget();
		}
		else {
			// Must be a raw JPA 1.0 MySpringPersistenceUnitInfo instance
	 ...
	 * JPA 1.0 based MySpringPersistenceUnitInfo object, adapting the <code>getSharedCacheMode</code>
	...
	private static class Jpa2PersistenceUnitInfoDecorator implements InvocationHandler {
		...
		private final MySpringPersistenceUnitInfo target;
		...
		public Jpa2PersistenceUnitInfoDecorator(MySpringPersistenceUnitInfo target) {
			this.target = target;
			try {
		...
		public final MySpringPersistenceUnitInfo getTarget() {
			return this.target;
		}
